...
def test_sync_income_includes_sec_quarterly_rows():
    service, db = make_service()
    ...
    service.provider.get_income_statements.return_value = [annual]
    service.provider.get_quarterly_income_statements.return_value = [quarterly]
    service.statement_repo.get_for_company.return_value = []
    with patch("quantcore.services.income_statement_service.FinancialPeriodMaterializer") as materializer:
...
def test_get_income_statements_supports_as_of():
    ...
    result = service.get_income_statements(
        "AAPL",
...
def test_sync_income_statements_updates_changed_observation_and_creates_revision():
    from types import SimpleNamespace

    service, db = make_service()
    company = make_company()
    service.security_repo.get_by_symbol.return_value = make_security(company)
    incoming = make_statement(date(2024, 9, 28), revenue=1000.0)
