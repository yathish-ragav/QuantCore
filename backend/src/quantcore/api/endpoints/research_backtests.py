...
from quantcore.api.dependencies import get_research_backtest_product_service
from quantcore.core.exceptions import InvalidInputError
from quantcore.schemas.research_backtests import (
    ResearchBacktestPeriodResponse,
...
router = APIRouter(
    ...
    tags=["Research Backtests"],
    dependencies=[Depends(require_scopes(RESEARCH_READ_SCOPE))],
)

MAX_RESEARCH_BACKTEST_ROWS = 1000


@router.post("", response_model=ResearchBacktestResponse)
def run_research_backtest(
    ...
    if requested_rows > MAX_RESEARCH_BACKTEST_ROWS:
        raise InvalidInputError(
            "Research backtest request exceeds the maximum of "
            f"{MAX_RESEARCH_BACKTEST_ROWS} symbol/as-of rows."
        )
    ...
    backtest_definition = ResearchBacktestDefinition(
        ...
        transaction_cost_identity=(
            request.cost_key,
            request.cost_definition_version,
        ),
    ...
    result = backtest_product_service.run(
    ...
    return ResearchBacktestResponse(
        ...
        target_portfolios=[
            ...
            for portfolio in result.target_portfolios
        ],
    )
