...
QuantCore will not treat one vendor as the truth for every dataset. This document distinguishes the current implementation from planned production source roles; a provider appearing in the strategy table does not by itself mean an adapter is currently implemented.
